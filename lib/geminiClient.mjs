...
export async function generateGeminiText({
  ...
  const deadline = now() + Math.max(Number(totalTimeoutMs) || 45000, 1000);
  ...
  for (let modelIndex = 0; modelIndex < models.length; modelIndex += 1) {
    ...
    for (let retry = 0; retry <= retryLimit; retry += 1) {
      ...
      try {
        ...
        if (/SAFETY|BLOCKED|PROHIBITED|RECITATION/.test(finishReason)) {
          throw new GeminiRequestError(
            `Gemini blocked its response. Finish reason: ${finishReason}`,
            {
              status: 422,
        ...
        if (finishReason === "MAX_TOKENS") {
          throw new GeminiRequestError(
            "Gemini stopped before completing the requested document",
            {
              status: 502,

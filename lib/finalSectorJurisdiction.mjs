...
} from "./nigeriaGeneralRegistry.mjs";
import {
  assessElectionViolenceRisk,
} from "./electionViolenceEligibility.mjs";
...
function preferredOfficialChannel(
  authority
) {
  const contact =
    authority?.contact || {};
  ...
  return (
    contact.submission_portal ||
    contact.complaints_portal ||
    contact.complaint_form ||
    contact.contact_form ||
    contact.incident_reporting_and_contact ||
    contact.communications_procedure ||
    contact.complaint_guidelines ||
    contact.registry_information ||
    contact.contact_guidance ||
    contact.contact_page ||
    contact.committee_page ||
    contact.delegation_page ||
...
function coordinatedEscalationRoute({
  authority,
  category,
  purpose,
  condition = "",
  ...
  return {
    ...
    condition,
    formalProcedure:
      authority.formal_procedure ===
      true,
    advocacyOnly:
      authority.advocacy_only ===
      true,
    submissionUrl:
      preferredOfficialChannel(
        authority
      ),
    contactEmails: [
      ...(authority.contact
...
function hasHumanRightsIssue(
  ...
  return assessElectionViolenceRisk(text).matched || includesAny(
    text,
    [
      "unlawful detention",
      "arbitrary detention",
...
function generalAuthorityChannel(
  authority
) {
  const contact =
    authority?.contact || {};
  ...
  return {
    deliveryMethod:
      authority?.dynamic_route
        ? "official_dynamic_or_physical_complaint_channel"
        : authority
            ?.verification
            ?.direct_email_verified
    ...
    submissionUrl:
      ...
      contact.contact_page ||
      contact.state_office_directory ||
      contact.escalation_guidance ||
      contact.website ||
      "",
...
export function resolveGeneralRouting(
  ...
  return {
    ...
    routingNote:
      "PCC is the primary general-complaint route. SERVICOM, NHRC and FCCPC are included only where the facts raise their specific public-service, human-rights or consumer-protection mandates. Where an internal institutional complaint channel exists, include the earlier complaint reference and response.",

    internalFirstAuthority:
      RELEVANT_MDA_INTERNAL_COMPLAINT_UNIT.name,

    internalEscalationGuidance:
      RELEVANT_MDA_INTERNAL_COMPLAINT_UNIT
        .contact
        .escalation_guidance,

    stateOfficeDirectory:
      PCC_STATE_OFFICE_DIRECTORY
        .contact
...
function internationalAuthorityChannel(
  authority
) {
  const contact =
    authority?.contact || {};
    ...
    deliveryMethod:
      authority?.portal_only
        ...
        : authority
            ?.verification
            ?.direct_email_verified
        ? "verified_email_or_official_correspondence_channel"
...
export function resolveInternationalRouting(
  ...
  if (
    electionRisk.matched &&
    electionRisk.sector ===
      "international_escalation"
  ) {
    const coordinatedRoutes = [
      coordinatedEscalationRoute({
        authority:
          OHCHR_SPECIAL_PROCEDURES,
        category:
          "formal_urgent_human_rights_communication",
        purpose:
          "Urgent UN communication concerning alleged threats to life, election violence and voter intimidation.",
      }),
      coordinatedEscalationRoute({
        authority:
          NHRC_FINAL_DOMESTIC_ESCALATION,
        category:
          "domestic_human_rights_protection",
        purpose:
          "Parallel domestic human-rights protection notice and complaint record.",
      }),
      coordinatedEscalationRoute({
        authority:
          INEC_ELECTION_INCIDENT_REPORTING,
        category:
          "election_management_incident_report",
        purpose:
          "Notify the election-management authority of alleged intimidation, suppression or election-related violence.",
      }),
      coordinatedEscalationRoute({
        authority:
          ACHPR_NON_STATE_COMMUNICATIONS,
        category:
          "african_union_human_rights_communication",
        purpose:
          "African Charter communication for alleged rights violations.",
        condition:
          "Follow the Commission's admissibility and supporting-evidence requirements.",
      }),
      coordinatedEscalationRoute({
        authority:
          ECOWAS_COURT_FORMAL_APPLICATION,
        category:
          "ecowas_formal_legal_route",
        purpose:
          "Regional judicial route where the facts, standing and filing requirements support a formal case.",
        condition:
          "Not an email petition; obtain legal advice and comply with the Court Rules and Registry procedure.",
      }),
      coordinatedEscalationRoute({
        authority:
          US_TLHRC_HUMAN_RIGHTS_ADVOCACY,
        category:
          "united_states_congressional_advocacy",
        purpose:
          "Evidence-based human-rights advocacy notice to the United States congressional commission.",
      }),
      coordinatedEscalationRoute({
        authority:
          UK_FOREIGN_AFFAIRS_COMMITTEE_ADVOCACY,
        category:
          "united_kingdom_parliamentary_advocacy",
        purpose:
          "Systemic-policy evidence for United Kingdom parliamentary scrutiny.",
        condition:
          "Use for a documented systemic pattern, not as an individual complaint-resolution service.",
      }),
      coordinatedEscalationRoute({
        authority:
          UK_FCDO_POLICY_CORRESPONDENCE,
        category:
          "united_kingdom_diplomatic_advocacy",
        purpose:
          "Foreign-policy correspondence concerning documented election and human-rights risks.",
      }),
      coordinatedEscalationRoute({
        authority:
          EU_DROI_HUMAN_RIGHTS_ADVOCACY,
        category:
          "european_union_parliamentary_advocacy",
        purpose:
          "Human-rights advocacy notice for European Parliament scrutiny.",
      }),
      coordinatedEscalationRoute({
        authority:
          EU_DELEGATION_NIGERIA_HUMAN_RIGHTS,
        category:
          "european_union_diplomatic_advocacy",
        purpose:
          "Local diplomatic human-rights and election-risk notice to the EU Delegation.",
      }),
      coordinatedEscalationRoute({
        authority:
          CANADA_GLOBAL_AFFAIRS_ADVOCACY,
        category:
          "canadian_diplomatic_advocacy",
        purpose:
          "Evidence-based foreign-policy and human-rights advocacy notice.",
      }),
      coordinatedEscalationRoute({
        authority:
          FRANCE_EMBASSY_NIGERIA_ADVOCACY,
        category:
          "french_diplomatic_advocacy",
        purpose:
          "Evidence-based diplomatic advocacy notice through France's official mission in Nigeria.",
        condition:
          "Advocacy notice only; the embassy is not represented as an adjudicator of an individual complaint.",
      }),
    ];
    ...
    return {
      ...
      sector:
        "international_escalation",
      caseType:
        "urgent_election_human_rights_communication",
      jurisdiction:
        "un_special_procedures_with_parallel_domestic_protection",
      routeKey:
        "election_violence_urgent_international_escalation",
      primaryInstitution:
        OHCHR_SPECIAL_PROCEDURES.name,
      ccInstitutions: [
        NHRC_FINAL_DOMESTIC_ESCALATION.name,
        INEC_ELECTION_INCIDENT_REPORTING.name,
      ],
      deliveryMethod:
        "official_secure_submission_portal_and_verified_parallel_domestic_email",
      emailRoutingExpected:
        true,
      contactEmails: [
        ...NHRC_FINAL_DOMESTIC_ESCALATION.contact.emails,
        ...INEC_ELECTION_INCIDENT_REPORTING.contact.emails,
      ],
      contactAddress:
        OHCHR_SPECIAL_PROCEDURES.contact.address,
      documentPurpose:
        "Urgent, evidence-based communication concerning alleged election violence, voter intimidation or threats to life",
      routingNote:
        "Use one verified evidence dossier with a separate recipient-specific cover note for each listed route; do not mass-CC institutions. Describe allegations as allegations, identify original evidence and witnesses, and do not rely exclusively on media reports. Formal procedures retain their own eligibility, admissibility and filing rules.",
      submissionUrl:
        OHCHR_SPECIAL_PROCEDURES
          .contact
          .submission_portal,
      parallelDomesticSubmissionUrl:
        NHRC_FINAL_DOMESTIC_ESCALATION
          .contact
          .complaint_form,
      sourceUrls:
        [
          ...new Set(
            coordinatedRoutes.flatMap(
              route =>
                route.sourceUrls
            )
          ),
        ],
      coordinatedEscalationPlan: {
        version: "1.0.0",
        strategy:
          "recipient_specific_parallel_submissions",
        routes:
          coordinatedRoutes,
        exclusions: [
          {
            ...
            reasonCode:
              "complainant_reports_security_force_failure_or_lack_of_confidence",
            note:
              "The police route is omitted from this coordinated plan when the complaint itself reports security-force failure, complicity, retaliation risk or a credible lack of confidence. This is a safety-based routing decision, not a factual finding against the institution.",
          },
        ],
        instructions:
          "Submit separately through each official channel. Tailor the cover note to the recipient, preserve submission receipts, and do not expose witnesses or sensitive evidence unnecessarily.",
      },
      safetyNotice:
        "If an attack is underway or a person faces immediate danger, contact the appropriate emergency or protection authority immediately rather than waiting for a petition response.",
      electionRisk,
    };
  }
  ...
  if (
    ...
    return {
      ...
      routingNote:
        "This is a non-binding advocacy route, not a court, appeal or enforceable legal remedy. Use one tailored submission through the verified official channel, clearly state the domestic steps already taken, and do not send bulk or repetitive messages.",
    };
  }

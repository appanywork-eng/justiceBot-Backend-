...
const BASE_NIGERIAN_POWER_PROVIDERS = [
  {
    key: "aedc",
    name:
...
const POWER_PROVIDER_CATALOGUE_BY_NAME =
  new Map(
    (
      POWER_CATALOGUE.players ||
      []
    ).map(
      provider => [
        provider.name,
...
export const NIGERIAN_POWER_PROVIDERS =
  Object.freeze(
    BASE_NIGERIAN_POWER_PROVIDERS.map(
      provider => {
        const catalogueRecord =
          POWER_PROVIDER_CATALOGUE_BY_NAME.get(
            provider.name
          );

        if (!catalogueRecord) {
          throw new Error(
            `Missing verified power catalogue record: ${provider.key}`
          );
        }

        const catalogueContact =
          catalogueRecord.contact || {};

        const catalogueVerification =
          catalogueRecord.verification || {};

        const emails =
          Array.isArray(
            catalogueContact.emails
          )
            ? [
                ...catalogueContact.emails,
              ]
        ...
        if (!emails.length) {
          throw new Error(
            `Verified power provider has no email: ${provider.key}`
          );
        }

        if (!sourceUrls.length) {
          throw new Error(
            `Verified power provider has no official sources: ${provider.key}`
          );
        }
        ...
        return Object.freeze({
          ...
          contact:
            Object.freeze({
              ...
              address:
                catalogueContact.address ||
                "",

              website:
                catalogueContact.website ||
                "",
...
if (
  ...
  throw new Error(
    `Expected 11 Nigerian DisCos, found ${NIGERIAN_POWER_PROVIDERS.length}`
  );
}

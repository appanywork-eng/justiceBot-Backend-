...
export function institutionNameIsExplicitlySupplied(
  ...
  const acronym = String(candidate || "")
    .match(/\(([A-Z][A-Z0-9]{1,9})\)/)?.[1];

  if (
    acronym &&
    ` ${narrative} `.includes(` ${normalize(acronym)} `)
  ) {
    return true;
  }
...
export function applyOversightRecipientPolicy({
  ...
  const NDPC = "Nigeria Data Protection Commission (NDPC)";

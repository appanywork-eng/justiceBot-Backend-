...
export async function updateFirebaseUserStatus({
  uid,
  disabled,
} = {}) {
  ...
  if (!cleanUid || cleanUid.length > 128) {
    throw new FirebaseIdentityError("A valid Firebase user ID is required.", {
      status: 400,
      code: "firebase_user_id_invalid",
    });
  }

  if (typeof disabled !== "boolean") {
    throw new FirebaseIdentityError("The disabled setting must be true or false.", {
      status: 400,
      code: "firebase_user_status_invalid",
    });
  }

  const user = await getFirebaseAuthClient().updateUser(cleanUid, { disabled });

  return {
    uid: String(user.uid || cleanUid),

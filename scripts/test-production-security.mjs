...
assert.match(server, /consumePdfLimit/);
assert.match(server, /if\s*\(!adminOk\)\s*\{\s*const generationLimit\s*=\s*await consumeGenerationLimit/);
assert.match(server, /await getUnlocked\(\s*validation\.tx_ref\s*\)/);
assert.match(server, /Unlock this petition before downloading its PDF/);
assert.doesNotMatch(server, /download-pdf"[\s\S]{0,250}req\.body\?\.text/);
...
assert.match(cloudBuild, /_GEMINI_MODEL:\s*gemini-3\.6-flash/);
assert.match(cloudBuild, /_GEMINI_TOTAL_TIMEOUT_MS:\s*'45000'/);
assert.match(cloudBuild, /_GEMINI_MAX_RETRIES:\s*'1'/);
assert.match(cloudBuild, /GEMINI_TOTAL_TIMEOUT_MS=\$\{_GEMINI_TOTAL_TIMEOUT_MS\}/);
assert.match(cloudBuild, /GEMINI_MAX_RETRIES=\$\{_GEMINI_MAX_RETRIES\}/);
assert.match(cloudBuild, /AI_SECTOR_CLASSIFY=false/);
...
assert.match(environmentExample, /^GEMINI_MODEL=gemini-3\.6-flash$/m);
assert.match(environmentExample, /^GEMINI_TOTAL_TIMEOUT_MS=45000$/m);
assert.match(environmentExample, /^GEMINI_MAX_RETRIES=1$/m);
assert.match(environmentExample, /^PDF_RATE_LIMIT_MAX=20$/m);
...
console.log("✅ PDF DOWNLOAD REQUIRES AN OWNED UNLOCKED PETITION AND IS RATE LIMITED");
console.log("✅ API SECURITY HEADERS ARE ENFORCED");
...
console.log("✅ AI GENERATION IS CAPPED BELOW THE WEBSITE GATEWAY TIMEOUT");

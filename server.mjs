...
const m =
petitionText.match(/^\ssubject\s:\s*(.+)\s*$/im) ||
petitionText.match(/^\sre\s:\s*(.+)\s*$/im);
...
role: "system",
content:
Classify this complaint into exactly ONE sector from: ${ALLOWED_SECTORS.join(   ", "   )}. Respond with ONLY the sector name.,
},
...
Sector: ${sector} | Case: ${caseType},   },   { role: "user", content: Complaint: ${complaint}` },
],
});

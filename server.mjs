...
app.use(
  cors({
    origin: "*", // Wide open for testing - change to your Netlify domain later
...
app.post("/generate-petition", async (req, res) => {
  ...
  try {
    ...
    // FIXED: Calculate adminCC before using it
...
Sector: ${sector} | CaseType: ${caseType}`.trim(),
    ...
    res.status(500).json({ error: "Failed to generate petition.", details: err.message });
  }
});
